...
LIST_PAGE_URL = f"{BASE_URL}/index?date=2026-01-02&page="
...
def get_all_links_url(max_limit):
    """
    Collects article links from the Metro TV News index pages using pagination.
    """
    all_links = set()
    page_counter = 1
    LINK_SELECTOR = 'a[href*="/read/"]'     # Link selector for Metro TV News
...
def scrape_full_article(article_url):
    ...
    TITLE_SELECTOR = 'h1' 
    CATEGORY_SELECTOR = 'div.breadcrumb-content p'  
    DATE_CONTAINER_SELECTOR = 'p.pt-20.date'
    ARTICLE_SELECTOR = 'div.news-text' 
    ...
    try:
        ...
        if date_container:
            ...
            # Ex: "M. Iqbal Al Machmudi • 6 January 2026 09:23"
            # Separate the data by dot
...
if final_scraped_data:
    ...
    df.to_csv('test_news_metrotvnews.csv', index=False, encoding='utf-8-sig')
    print(f"\nSuccess! Scraped a total of {len(final_scraped_data)} full articles and saved to test_news_metrotvnews.csv")
else:
